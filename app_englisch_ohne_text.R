...
#setwd("/Users/leonardmichelsfau/Dropbox (Uni Bamberg)/01_FAU_Team/01_Projects_Ongoing/COV19_LMI/02_Implementation_development/covid19_simulation_without_nudge")
if (FALSE) {
...
ui <- shinyUI(
  fluidPage(
    ...
    titlePanel(div(
      HTML(
        "<b style=\"color:DodgerBlue;\">COVID-19 Infection Probability Calculator</b>"
      )
    )),
    ...
    fluidRow(
      ...
      column(4, sliderInput(
        "zeit",
        "Duration of the gathering (in h):",
        1, 16, 6
      )),
      ...
      sidebarPanel(
        ...
        selectInput(
          ...
          c(
            ...
            "Never" = "NoAir",
            "Windows permanently tilted" = "FensterGekippt",
            "Regular shock ventilation (10min/h)" = "Stoßlueften"
...
server <- function(input, output, session) {
  ...
  output$infoText <- renderText({
    ...
    if (people_infected < 0.5) {
      ...
      if (input$air == "NoAir") {
        ...
        luft <- "Windows permanently tilted"
      } else if (input$air == "Stoßlueften") {
        luft <- "Regular shock ventilation (10min/h)"
      }
      
      if (input$distance == "k1m") {
        abstand <- "Less than 1.5m"
      } else if (input$distance == "m1m") {
        abstand <- "More than 1.5m"
      }
      
      if (input$masks == "No") {
        maske <- "None"
      } else if (input$masks == "OP") {
        maske <- "Surgical masks"
      } else if (input$masks == "FFP2") {
        maske <- "FFP2 masks"
      }
      
      paste(
        "If the persons present comply with the ","<b>", "selected measures (", "Distance: ",
        abstand,
        ", ", "Masks: ",
        maske,", " ,"Airing: ",
        "",
        luft,
        ")","</b>"," the probability that at least one person in the room will become infected with COVID-19 is ", "<b>",
        format(risk_of_1_person_in_room_being_infected, digits = 3),
        "%", "</b>",
        ". The probability of ", "<b>", "you personally ", "</b>", "becoming infected is ", "<b>",
        format(infect_risk_individual, digits = 3),
        "%"  , "</b>",
        ". On average, " , "<b>",
        format(people_infected, digits = 1) ,
        " other person(s)", "</b>", " in the room will become infected with COVID-19.", sep = ""
      )
  ...
  output$attention <- renderText({
    ...
    if (input$distance == "k1m" &&
        input$masks == "No" && input$air == "NoAir") {
      paste(
        "<strong style=\"font-size:30px;\">",
        "<font style=\"color:Red;\">",
        "Nicht von Wissenschaftlern empfohlene Maßnahmenwahl!",
        "</font>",
        "</strong>"
      )
    } else if (risk_of_1_person_in_room_being_infected > 50 ||
               infect_risk_individual > 30) {
      
    }
    
    else if (risk_of_1_person_in_room_being_infected <= 10 &&
             infect_risk_individual <= 10 &&
             (
               input$distance == "m1m" ||
               input$masks == "FFP2" ||
               input$masks == "OP" ||
               input$air == "FensterGekippt" || input$air == "Stoßlueften"
             )) {
      paste(
        "<strong style=\"font-size:30px;\">",
        "<font style=\"color:Green;\">",
        "Von Wissenschaftlern empfohlene Maßnahmen!",
        "</font>",
        "</strong>"
      )
    } else if (input$distance == "m1m" &&
               input$masks == "FFP2" && input$air == "Stoßlueften") {
      paste(
        "<strong style=\"font-size:30px;\">",
        "<font style=\"color:Green;\">",
        "Von Wissenschaftlern empfohlene Maßnahmen!",
      ...
      paste(
        ...
        "Maßnahmenwahl trägt mäßig zur Reduktion bei!",
        "</font>",
        "</strong>"
      )
    }
  ...
  output$text6 <- renderText({
    ...
    paste(
      ...
      "</b>",
      "<strong style=\"font-size:50px;\">",
      "<font style=\"color:DodgerBlue;\", padding-left=\"10em\">",
      format(risk_of_1_person_in_room_being_infected, digits = 3),
      "%",
      "</font>",
      "</strong>"
  ...
  output$text2 <- renderPlot({
    ...
    df <-
      data.frame(
        choices = c("None", "Your selection",  "All"),
    ...
    p <-
      p + labs(x = "Selected measures", y = "Your personal infection probability") + scale_y_continuous(
        labels = function(x)
          paste0(x * 100, "%"),
        limits = c(0, 1)
