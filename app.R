...
ui <- shinyUI(
  fluidPage(
    ...
    titlePanel(div(
      HTML(
        "<b style=\"color:DodgerBlue;\">COVID-19 Infektionsrechner</b>"
      )
    )),
  ...
  output$infoText <- renderText({
    ...
    if (people_infected < 0.5) {
      ...
      if (input$air == "NoAir") {
        ...
        luft <- "Fenster dauerhaft gekippt"
      } else if (input$air == "Stoßlueften") {
        luft <- "Regelmäßiges Stoßlüften (10min/h)"
      }
      
      if (input$distance == "k1m") {
        abstand <- "Weniger als 1,5m"
      } else if (input$distance == "m1m") {
        abstand <- "Mehr als 1,5m"
      }
      ...
      paste(
        "Wenn die anwesenden Personen die ","<b>", "ausgewählten Maßnahmen (", "Abstand: ",
        abstand,
        ", ", "Masken: ",
        maske,", " ,"Lüftung: ",
        "",
        luft,
        ")","</b>"," einhalten, beträgt die Wahrscheinlichkeit, dass sich mindestens eine Person im Raum mit COVID-19 ansteckt ", "<b>",
        format(risk_of_1_person_in_room_being_infected, digits = 3),
        "%", "</b>",
        ". Die Wahrscheinlichkeit, dass Sie persönlich  infizieren, liegt bei ", "<b>",
        format(infect_risk_individual, digits = 3),
        "%"  , "</b>",
        ". Im Durchschnitt infizieren sich " , "<b>",
        format(people_infected, digits = 1) ,
        " weitere Person(en)", "</b>", " im Raum mit COVID-19.", sep = ""
      )
